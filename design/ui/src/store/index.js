  ...
  actions: {
    async start({ dispatch, getters, state }) {
      ...
      axios.interceptors.request.use(async (config) => {
        ...
        console.debug(
          "Request is waiting.",
          state.user ? state.user.pk : state.user,
          getters.selectedOrganization
            ? getters.selectedOrganization.pk
            : getters.selectedOrganization
        );
    ...
    loadOrganizations(context) {
      axios
        .get("/api/organization/", { privileged: true })
        .then((response) => {
          let organizations = {};
          ...
          if (
            !(context.state.selectedOrganizationId in organizations) &&
            response.data.length > 0
          ) {
            context.commit("setSelectedOrganizationId", {
    ...
    async loadBasicInfo({ dispatch, commit }) {
      try {
        ...
        dispatch("showSnackbar", {
          content: "Error loading basic info: " + error,
          color: "error",
        });
      }
    },
