...
<template>
    ...
    <table class="map">
        <tr>
            <th></th>
            <th
                    v-for="[chpk, chart] of charts.entries()" :key="'chart-' + chpk"
                    class="rotated"
            >
        ...
        <tr v-for="[vpk, reportView] of reportViews.entries()" :key="vpk">
            ...
            <td
                    v-for="[chpk, chart] of charts.entries()" :key="vpk + '-' + chpk"
                    class="text-center"
            >
                <div
                        v-if="reportViewToChart.has(`${vpk}-${chpk}`)"
                >
                    <div
                            class="position"
                            @click="activateLinkEdit(vpk, chpk)"
                    >
        ...
        <tr>
            <th></th>
            <th
                    v-for="[chpk, chart] of charts.entries()" :key="'b-chart-' + chpk"
                    class="rotated-down"
            >
                <div><span>{{ chart.name }}</span></div>
            </th>
        ...
        >
            <v-card>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn @click="showEditDialog=false">{{ $t('actions.close') }}</v-btn>
                    <v-btn
                            @click="storeReportViewToChart(editedReportView.pk, editedChart.pk, editedPosition); showEditDialog=false"
                            :disabled="forbiddenPositions.has(Number.parseInt(editedPosition))"
                            color="success"
                    >{{ $t('actions.save') }}</v-btn>
                    <v-btn
                            @click="removeReportViewToChart(editedReportView.pk, editedChart.pk); showEditDialog=false"
                            color="error"
                    >
                        {{ $t('actions.delete') }}
                    </v-btn>
  ...
  methods: {
    ...
    async fetchReportViewToChartData () {
      try {
        ...
        for (let rec of result.data) {
          this.reportViewToChart.set(`${rec.report_data_view}-${rec.chart_definition}`, rec)
        }
      } catch (error) {
        this.showSnackbar({
    ...
    async storeReportViewToChart (viewId, chartId, position) {
      ...
      if (this.reportViewToChart.has(key)) {
        ...
        try {
          await axios.patch(
            this.reportViewToChartEndpointURL + link.pk + '/',
            {position: position}
          )
        ...
        try {
          let resp = await axios.post(
            this.reportViewToChartEndpointURL,
            {
          ...
          this.reportViewToChart = new Map(this.reportViewToChart.set(`${resp.data.report_data_view}-${resp.data.chart_definition}`, resp.data))
        } catch (error) {
          this.showSnackbar({
    ...
    activateLinkEdit (viewId, chartId) {
      ...
      this.editedPosition = this.reportViewToChart.get(`${viewId}-${chartId}`).position
      this.forbiddenPositions = new Set(Array.from(this.reportViewToChart.values()).filter(item => item.report_data_view == viewId && item.position != this.editedPosition).map(item => item.position))
