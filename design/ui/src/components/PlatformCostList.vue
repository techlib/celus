...
en:
  ...
  weights_tooltip:
    When more than one interest is defined for a platform, you can use the
...
cs:
  ...
  weights_tooltip:
    Pokud je pro platformu definováno více druhů zájmu, můžete použít následující
  ...
  <v-container fluid class="pt-0">
    ...
    <v-row>
      ...
      <v-col
        ...
        sm="3"
        md="2"
        v-for="group in activeInterestGroups"
...
export default {
  ...
  computed: {
    ...
    platformToInterest() {
      ...
      this.interestData
        .filter((item) => item.date__year == this.selectedYear)
        .forEach((item) => (platformIdToInterest[item.platform] = item));
      return platformIdToInterest;
    },
    platformToPayment() {
      let platformToPayment = {};
      this.paymentData
        .filter((item) => item.year == this.selectedYear)
        .forEach((item) => (platformToPayment[item.platform] = item.price));
    ...
    platformData() {
      ...
      data.forEach((item) => {
        ...
        if (item.yearInterest) {
          for (let ig of this.activeInterestGroups) {
            const weight =
              ig.short_name in this.interestWeights
                ? this.interestWeights[ig.short_name]
                : 1;
            totalInterestWeighted +=
              ig.short_name in item.yearInterest
                ? weight * item.yearInterest[ig.short_name]
                : 0;
            totalInterestPlain +=
              ig.short_name in item.yearInterest
                ? item.yearInterest[ig.short_name]
          ...
          for (let ig of this.activeInterestGroups) {
            const weight =
              ig.short_name in this.interestWeights
                ? this.interestWeights[ig.short_name]
                : 1;
            pricePerUnitInterest[ig.short_name] =
              (item.price * weight) / totalInterestWeighted;
          }
        }
        item.totalInterest = totalInterestWeighted;
        item.pricePerUnit =
          item.price && totalInterestPlain
            ? item.price / totalInterestPlain
            : null;
  ...
  methods: {
    ...
    async fetchInterest() {
      if (this.interestUrl) {
        try {
          ...
          this.showSnackbar({
            content: "Error loading available years: " + error,
            color: "error",
          });
        }
      }
    ...
    async fetchPayments() {
      if (this.paymentsUrl) {
        try {
          ...
          this.showSnackbar({
            content: "Error loading platform data: " + error,
            color: "error",
          });
        }
      }
    ...
    async savePrice({ price }) {
      ...
      for (let payment of this.paymentData) {
        if (
          payment.organization === this.selectedOrganizationId &&
      ...
      if (paymentObject) {
        try {
          ...
          this.showSnackbar({
            content: "Error saving new price: " + error,
            color: "error",
          });
        }
      } else {
