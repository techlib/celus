...
en:
    step1: Hello
    step1_text: Hello, it looks like you are new to Celus and there are no data in your account.
    ...
    step2_text_3: This is what the dialog to enter SUSHI credentials looks like in Celus, to give you
        an idea of what you need to have ready.
    step3: Extras
    step3_text: Sometimes the provider will give you more credentials than the simple pair of
    ...
    step4_text: After you put your credentials into Celus, we strongly advise to test them
        ...
        <img src="../../assets/sushi_credentials-save-and-test-btn.png" alt="Save and test credentials">
    ...
    step6: Let's do it
    step6_text: OK, you should now know everything that is needed to get you started. Click the
...
<template>
    <v-stepper v-model="step" non-linear>
        <v-stepper-header>
            <v-stepper-step :step="1" :complete="step > 1" editable edit-icon="fa-check fa-small">
                {{ $t('step1') }}
            </v-stepper-step>
            <v-stepper-step :step="2" :complete="step > 2" editable edit-icon="fa-check fa-small">
                {{ $t('step2') }}
            </v-stepper-step>
            <v-stepper-step :step="3" :complete="step > 3" editable edit-icon="fa-check fa-small">
                {{ $t('step3') }}
            </v-stepper-step>
            <v-stepper-step :step="4" :complete="step > 4" editable edit-icon="fa-check fa-small">
                {{ $t('step4') }}
            </v-stepper-step>
            <v-stepper-step :step="5" :complete="step > 5" editable edit-icon="fa-check fa-small">
                {{ $t('step5') }}
            </v-stepper-step>
            <v-stepper-step :step="6" :complete="step > 6" editable edit-icon="fa-check fa-small">
                {{ $t('step6') }}
            </v-stepper-step>
        </v-stepper-header>
        ...
        <v-stepper-items>
            ...
            <v-stepper-content :step="6">
                ...
                <p class="text-center pt-5">
                    <v-btn @click="showCredentialsDialog = true" color="primary">{{ $t('Add credentials') }}</v-btn>
                </p>
            </v-stepper-content>
        </v-stepper-items>
    </v-stepper>
