...
<template>
  <div class="d-flex align-start">
    <table
      class="overview text-disabled"
      :style="{ minWidth: twoPanes ? '25vw' : '100%' }"
    >
      <tr v-if="report.description && !twoPanes">
        <th class="align-top">{{ $t("labels.description") }}:</th>
    ...
    <div v-if="twoPanes" class="overview text-disabled ml-12">
      ...
      <div v-if="report.description">
        <div class="bold">{{ $t("labels.description") }}:</div>
        <div class="ml-4">{{ report.description }}</div>
      </div>
    </div>
  </div>
</template>
