...
class TestReportType(object):
    ...
    def test_dimension_short_names_ordering(self):
        ...
        for i, dim in enumerate(dims):
            ...
            ReportTypeToDimension.objects.create(dimension=dim_obj, report_type=report,
                                                 position=i)
        ...
        for i, dim in enumerate(dims):
            dim_obj = Dimension.objects.create(short_name=dim+'x', name=dim+'x',
                                               type=Dimension.TYPE_INT)
            ReportTypeToDimension.objects.create(dimension=dim_obj, report_type=report2,
                                                 position=5-i)
