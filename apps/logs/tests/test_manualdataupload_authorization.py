...
def mdu_with_user_levels(platforms, report_type_nd, identity_by_user_type, organizations):
    def do_it(org):
        ...
        return MDUSet(rel_admin=mdu_rel_admin, unrel_admin=mdu_unrel_admin, master=mdu_master,
                      super=mdu_super)
    return do_it
...
class TestAuthorization(object):
    ...
    def test_mdu_get_object_api_access(
            self, user_type, can_access_unrel, can_access_rel,
            identity_by_user_type, client, organizations, authentication_headers, platforms,
            report_type_nd):
        identity, org = identity_by_user_type(user_type)
        rt = report_type_nd(0)
        ...
        for i, (mdu, can) in enumerate(((mdu_unrel, can_access_unrel),
    ...
    def test_mdu_delete_api_access(self, user_type,
                                   can_delete_rel_org_admin, can_delete_unrel_org_admin,
                                   can_delete_master, can_delete_superadmin,
                                   identity_by_user_type, client, authentication_headers,
    ...
    def test_annotation_modify_api_access(
            ...
            can_modify_master, can_modify_superadmin, identity_by_user_type, client,
            authentication_headers, platforms, mdu_with_user_levels):
        identity, org = identity_by_user_type(user_type)
        mdu_set = mdu_with_user_levels(org)
        for i, (mdu, can) in enumerate(
    ...
    def test_annotation_modify_organization_api_access(
            self, user_type, can_set_rel_org, can_set_unrel_org,
            identity_by_user_type, client, organizations, authentication_headers, report_type_nd,
            platforms):
        identity, org = identity_by_user_type(user_type)
        rt = report_type_nd(0)
