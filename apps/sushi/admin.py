...
def delete_with_data(modeladmin, request, queryset):
    ...
    messages.info(request, f'{attempts_deleted[0]} attempts deleted with '
                           f'{batches_deleted[0]} data batches')
...
class SushiFetchAttemptAdmin(admin.ModelAdmin):
    ...
    list_display = ['organization', 'platform', 'counter_version', 'report', 'timestamp',
                    'start_date', 'end_date',
                    'queued', 'download_success', 'processing_success',
                    'contains_data', 'import_crashed', 'is_processed', 'error_code',
                    'has_import_batch']
