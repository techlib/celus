...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--interest-4', dest='interest_4_col', default='Counter 4 - interest',
        ...
        parser.add_argument('--interest-5', dest='interest_5_col', default='Counter 5 - interest',
    ...
    def handle(self, *args, **options):
        ...
        report_type_code_to_obj = {(None, report_type.short_name): report_type
        ...
        with open(options['file'], 'r') as infile:
            ...
            for rec in reader:
                ...
                if platform.sushicredentials_set.count() == 0:
                    ...
                    continue
                for counter_ver, col_name, col_interest in ((4, c4_col_name, c4_col_interest),
                                                            (5, c5_col_name, c5_col_interest)):
                    rts = rec.get(col_name, '')
                    report_objs = []
                    if rts:
                        report_objs = self.report_list_str_to_objs(rts, rt_ver_code_to_obj,
                                                                   counter_ver, platform_code)
                    if report_objs:
                        sc_count = 0
        ...
        for platform_code, int_report_objs in platform_to_interest.items():
            if int_report_objs:
                ...
                self.stderr.write(self.style.WARNING(f'No known reports to add to '
                                                     f'platform {platform_code}: "{int_rts}"'))
